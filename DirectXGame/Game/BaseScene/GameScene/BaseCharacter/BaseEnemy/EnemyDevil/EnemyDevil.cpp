...
void EnemyDevil::Initialize(const YokosukaEngine* engine, const Camera3D* camera3d, const ModelHandleStore* modelHandleStore, const Vector3& position, int32_t hp)
{
	...
	for (uint32_t i = 0; i < kNumModel; i++)
	{
		...
		// モデルハンドルを読み込む
		models_[i].modelHandle_ = modelHandleStore_->GetModelHandle(ModelHandleStore::kEnemyDevil)[i];
	...
	// ステートの生成
	state_ = std::make_unique<EnemyDevilStateApproachingRear>(this);
}
...
/// </summary>
/// <returns></returns>
Vector3 EnemyDevil::GetBodyWorldPosition() const
{
	// ワールド座標
...
/// </summary>
/// <returns></returns>
AABB EnemyDevil::GetCollisionAABB() const
{
	// AABB
	AABB aabb;
...
void EnemyDevil::OnCollision(const BasePlayerBullet* playerBullet)
{
	// 基底クラスの衝突判定応答
...
void EnemyDevil::ChangeState(State state)
{
	switch (state)
	{
	case kApproachingRear:
		// 後方から
		state_ = std::move(std::make_unique<EnemyDevilStateApproachingRear>(this));
	...
	case kAwayTop:
		// 上方向に離脱
		state_ = std::move(std::make_unique<EnemyDevilStateAwayTop>(this));
