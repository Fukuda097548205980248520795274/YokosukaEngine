...
void EnemyStingray::Initialize(const YokosukaEngine* engine, const Camera3D* camera3d, const ModelHandleStore* modelHandleStore, const Vector3& position, int32_t hp)
{
	// 基底クラス初期化
	...
	for (uint32_t i = 0; i < kNumModel; i++)
	{
		...
		// モデルハンドルを読み込む
		models_[i].modelHandle_ = modelHandleStore_->GetModelHandle(ModelHandleStore::kEnemyStingray)[i];
...
/// </summary>
/// <returns></returns>
Vector3 EnemyStingray::GetBodyWorldPosition() const
{
	// ワールド座標
...
/// </summary>
/// <returns></returns>
AABB EnemyStingray::GetCollisionAABB() const
{
	// AABB
	AABB aabb;
...
void EnemyStingray::OnCollision(const BasePlayerBullet* playerBullet)
{
	// 基底クラスの衝突判定応答
...
/// ダメージカラー
/// </summary>
void EnemyStingray::DamageColor()
{
	// ダメージカラーをリセットする
	for (uint32_t i = 0; i < kNumModel; i++)
	{
