...
void EnemyJet::Initialize(const YokosukaEngine* engine, const Camera3D* camera3d, const ModelHandleStore* modelHandleStore, const Vector3& position, int32_t hp)
{
	// 基底クラス初期化
	...
	for (uint32_t i = 0; i < kNumModel; i++)
	{
		...
		// モデルハンドルを読み込む
		models_[i].modelHandle_ = modelHandleStore_->GetModelHandle(ModelHandleStore::kEnemyJet)[i];
...
/// </summary>
/// <returns></returns>
Vector3 EnemyJet::GetBodyWorldPosition() const
{
	// ワールド座標
...
/// </summary>
/// <returns></returns>
AABB EnemyJet::GetCollisionAABB() const
{
	// AABB
	AABB aabb;
...
void EnemyJet::OnCollision(const BasePlayerBullet* playerBullet)
{
	// 基底クラスの衝突判定応答
